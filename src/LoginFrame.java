...
System.out.print("hi");
        ...
        JOptionPane.showMessageDialog(this, "Password must be numeric.", "Format Error", JOptionPane.ERROR_MESSAGE);
 }      
...
private int CheackUserName() {
    for (int i = 0; i < ManagerWindow.sizeCustomar; ++i) {
        if (ManagerWindow.customar[i].getUserName().equals(userName)) {
            if (ManagerWindow.customar[i].getPassword() == realPassowrd) {
                if (ManagerWindow.customar[i].isIsInsert()) {
                    nameCustomar = ManagerWindow.customar[i].getUserName();
                    JOptionPane.showMessageDialog(this, "Welcome " + ManagerWindow.customar[i].getNameCustomer(), "Welcome", JOptionPane.INFORMATION_MESSAGE);
                    CustomarWindow.shopProduct.setEnabled(true);
                    CustomarWindow.viewCart.setEnabled(true);
                    return 1; // success
                } else {
                    JOptionPane.showMessageDialog(this, ManagerWindow.customar[i].getNameCustomer() + "\nYou haven't inserted yet :(", "Error", JOptionPane.ERROR_MESSAGE);
    ...
    // If loop ends and no username matched
